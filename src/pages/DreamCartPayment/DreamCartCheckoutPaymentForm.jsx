...
>>>>>>> abb057ee629e9a8f36411e2f96441363c545540b
import React, { useState, useEffect } from 'react';
import { loadStripe } from '@stripe/stripe-js';
import { accountService, alertService } from '@/_services';
<<<<<<< HEAD
...
export default function CheckoutForm() {
   ...
   useEffect(() => {
      ...
      setCampainID(localStorage.getItem('selected_campaign_id'));
      setCampainActualPrice(
         localStorage.getItem('selected_campaign_actual_price')
      );
      setCampainCashPaid(localStorage.getItem('selected_campaign_cash_paid'));
      setCampainDiscount(localStorage.getItem('discount_code'));
      setCampainDreamCoin(localStorage.getItem('dream_coins'));
      setCampainCount(localStorage.getItem('item_count_value'));
      setDiscountAmount(
         localStorage.getItem('selected_campaign_discount_amount')
      );
      if (!stripe) {
         return;
      }
...
=======
   ...
   const onStripToken = (token) => {
      const userDetails = {
         discountCode: '',
         campain_discount: parseFloat(campain_discount),
         campaignId: campain_id,
         actualPrice: parseFloat(campain_actual_price),
         discountAmount: parseFloat(discount_amount),
         dreamCoinsUsed: parseFloat('0'),
         cashPaid: parseFloat(campain_cash_paid),
         totalPurchasedCoupons: parseInt(campain_count),
         payment_token_id: token.id,
         type_of_payment: 'stripe',
         payment_instrument: 'card',
         payment_instrument_type: 'VISA',
         user_token: !!user ? user.jwtToken : '',
      };
      console.log(userDetails);
      alertService.clear();
      accountService
         .PurchaseCoupons(
            userDetails.discountCode,
            userDetails.campaignId,
            userDetails.actualPrice,
            userDetails.discountAmount,
            userDetails.dreamCoinsUsed,
...
>>>>>>> abb057ee629e9a8f36411e2f96441363c545540b
   ...
   return (
      <form
         ...
         className="responsive dreamcart-form"
      >
